...
// ---------------------------------------------------------------------------
// openBulkAddBucketAlternativesPr — admin "batch add alternatives" (2026-04-28)
// ---------------------------------------------------------------------------
// Adds N curated alternatives to etfs.ts in a single PR. Used by the
// /admin batch-alternatives panel which lets the operator paste a list
// of (parentKey, ISIN, draft) tuples and ship them as one reviewable
// commit instead of N separate PRs.
//
// Why one PR (not N):
//   - Reviewer fatigue: ten add-alt/* PRs against the same file pile up
//     conflicts; one PR with a clear "added"/"skipped" table is easier
//     to audit and merge.
//   - File-level conflicts: each per-row PR mutates the same etfs.ts —
//     merging them in any order forces github to recompute conflicts on
//     every merge. One PR sidesteps that entirely.
//
// Per-row outcomes (mirrors injectAlternative status names):
//   - ok                  — entry was inserted into the running content
//   - parent_missing      — `parentKey` not in the catalog
//   - duplicate_isin      — ISIN already used by some catalog/alt entry
//   - cap_exceeded        — parent already has 2 alternatives
//
// Idempotency: each injection runs against the running content, so an
// ISIN added in row 3 will be detected as `duplicate_isin` if row 7
// asks for the same ISIN under a different bucket.
//
// Aborts (without opening any branch/PR) when zero rows succeed — the
// caller should surface the per-row reasons to the operator.
// ---------------------------------------------------------------------------
export type BulkAlternativeStatus =
  | "ok"
  | "parent_missing"
  | "duplicate_isin"
  | "cap_exceeded";

export interface BulkAlternativeRow {
  parentKey: string;
  entry: NewAlternativeEntry;
}

export interface BulkAlternativeOutcome {
  parentKey: string;
  isin: string;
  name: string;
  status: BulkAlternativeStatus;
  conflict?: string;
}

export async function openBulkAddBucketAlternativesPr(args: {
  rows: BulkAlternativeRow[];
}): Promise<{
  url: string;
  number: number;
  outcomes: BulkAlternativeOutcome[];
  added: BulkAlternativeOutcome[];
  skipped: BulkAlternativeOutcome[];
}> {
  if (!githubConfigured()) {
    throw new Error(
      "GitHub PR creation is not configured. Set GITHUB_PAT, GITHUB_OWNER, GITHUB_REPO.",
  ...
  // 1. Read the current etfs.ts on the base branch — once. All N
  //    injections operate against the running mutated content so
  //    duplicate-ISIN checks roll forward across rows.
  const { data: baseRef } = await octokit.git.getRef({
    owner,
    repo,
    ref: `heads/${base}`,
  ...
  let runningContent = Buffer.from(fileMeta.content, "base64").toString("utf8");

  // 2. Apply each injection in order. Track per-row outcome.
  const outcomes: BulkAlternativeOutcome[] = [];
  for (const row of args.rows) {
    const result = injectAlternative(runningContent, row.parentKey, row.entry);
    let status: BulkAlternativeStatus;
  ...
  const added = outcomes.filter((o) => o.status === "ok");
  const skipped = outcomes.filter((o) => o.status !== "ok");
  if (added.length === 0) {
    throw new Error(
      "Bulk add-alternative PR aborted: every row was rejected (parent missing, duplicate ISIN, or cap exceeded). Check the per-row reasons.",
    );
  }

  // 3. Create the branch. Stamped name (UTC YYYYMMDDHHmmss) so consecutive
  //    runs don't collide; the human-readable count keeps the GitHub UI
  //    list scannable.
  const stamp = new Date()
    .toISOString()
    .replace(/[-:T]/g, "")
  ...
  // 4. Single commit with all N successful injections.
  const commitTitle =
    added.length === 1
      ? `Add ${added[0].name} (${added[0].isin}) as alternative under ${added[0].parentKey}`
      : `Add ${added.length} alternatives to ETF catalog (bulk)`;
  await octokit.repos.createOrUpdateFileContents({
    owner,
    repo,
    path: ETFS_FILE_PATH,
    branch,
    message: commitTitle,
    content: Buffer.from(runningContent, "utf8").toString("base64"),
    sha: fileMeta.sha,
  });

  // 5. Open the PR. Body lists every added/skipped row so the reviewer
  //    can spot surprises quickly.
  const addedLines = added.map(
    (o) => `- \`${o.parentKey}\` ← \`${o.isin}\` — ${o.name}`,
  );
  const skippedLines = skipped.map((o) => {
    const reason =
      o.status === "parent_missing"
        ? "parent bucket missing"
        : o.status === "duplicate_isin"
          ? `ISIN already used by ${o.conflict ?? "another entry"}`
          : "parent already at the 2-alt cap";
    return `- \`${o.parentKey}\` ← \`${o.isin}\` — ${o.name} (${reason})`;
  });
  const body = [
    `Bulk-adds **${added.length}** curated alternatives to \`${ETFS_FILE_PATH}\`.`,
    "",
    "Generated from `/admin` → **Batch — Alternativen hinzufügen**. Each row was scraped from justETF and validated server-side before this PR was opened.",
    "",
    "**Added**",
    ...addedLines,
    ...(skippedLines.length > 0
      ? ["", "**Skipped (will not land in this PR)**", ...skippedLines]
      : []),
    "",
    "**Reviewer checklist**",
    "- Confirm each `parentKey` is the correct bucket for that ETF.",
    "- Confirm `defaultExchange` matches the preferred listing per row.",
    "- Confirm there are no duplicate ISINs across the diff.",
    "",
    "After merge + redeploy each ISIN appears in the Build tab's per-bucket ETF picker. The companion look-through PR (if opened in the same batch) makes the Methodology swap view show full holdings/geo/sector data on day 1.",
  ].join("\n");
