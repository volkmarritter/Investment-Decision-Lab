...
export const TRANSLATIONS: Record<Lang, Record<string, string>> = {
  en: {
    ...
    "build.lookThrough.desc": "Decompose selected ETFs into their underlying country, sector and top-holding exposures. When ON, this also routes the Risk & Performance metrics (Vol, Beta, TE, Alpha, TE-Contribution) and the Correlation Matrix in the Methodology tab via the actual ETF holdings — e.g. iShares Core MSCI Europe is split into ~20 % UK + ~14 % CH + continental EU rather than treated as a single Europe bucket. When OFF, those metrics fall back to row-region routing and the Look-Through panel is hidden.",
  ...
  de: {
    ...
    "build.lookThrough.desc": "Zerlegt die gewählten ETFs in ihre zugrundeliegenden Länder-, Sektor- und Top-Holding-Engagements. Wenn AN, werden zusätzlich die Risiko- & Performance-Kennzahlen (Vola, Beta, TE, Alpha, TE-Beitrag) sowie die Korrelationsmatrix im Methodik-Tab über die tatsächlichen ETF-Bestände geroutet — z. B. wird iShares Core MSCI Europe in ~20 % UK + ~14 % CH + Kontinental-EU aufgeteilt, statt als ein Europe-Bucket behandelt zu werden. Wenn AUS, fallen diese Kennzahlen auf das Zeilen-Region-Routing zurück und das Look-Through-Panel wird ausgeblendet.",
