...
export function buildPortfolio(
  ...
  const learning = de
    ? [
        ...
        !input.includeCurrencyHedging && input.hedgeForeignBonds !== false && input.baseCurrency !== "USD" && bondsPct > 0 ? "Anleihen-Hedging: Fremdwährungsanleihen sind in die abgesicherte Anteilsklasse geroutet — FX-Volatilität wird im Anleihen-Sleeve eliminiert (Sharpe-relevant), gegen ~15 Bp Mehrkosten und Verlust des Zinsdifferentials." : "",
    ...
    : [
        ...
        !input.includeCurrencyHedging && input.hedgeForeignBonds !== false && input.baseCurrency !== "USD" && bondsPct > 0 ? "Bond Hedging: Foreign-currency bonds are routed to the hedged share class — FX volatility is removed from the bond sleeve (a Sharpe win) at a cost of ~15 bps and the interest-rate differential." : "",
