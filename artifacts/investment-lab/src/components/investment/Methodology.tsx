      ...
      >
        ...
        >
          ...
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              {de
                ? "Damit eine Position niemals stillschweigend aus der Geo-/Sektor-/Einzeltitel-Aggregation herausfällt, wird die Look-Through-Abdeckung an drei Stellen erzwungen: (1) **Im Katalog selbst** — jede Default-ISIN muss ein Look-Through-Profil haben, sonst lässt sich der Katalog gar nicht ausliefern; für Alternativen und Pool-Einträge wird die Lücke pro Bucket angezeigt. (2) **In den Admin-Add-Flows** — bevor ein neues Instrument als Default, Alternative oder Pool-Eintrag gespeichert wird, prüft die App live bei justETF, ob Länder- und Sektor-Daten geliefert werden; fehlen sie, wird das Speichern blockiert (eine Force-Option erlaubt es bewusst, die Position trotzdem aufzunehmen — sie erscheint dann unter „nicht zugeordnete ETFs“). (3) **Im Erklären-Tab** — wenn eine ISIN ausserhalb des Katalogs eingegeben wird, holt die App das Look-Through-Profil automatisch live von justETF nach. Bei Erfolg verschwindet die rote „Look-through-Daten fehlen“-Warnung sofort, ohne Neuladen. Bei Fehlschlag (justETF nicht erreichbar oder kein Geo-/Sektor-Block verfügbar) erscheint eine deutliche Fehlermeldung mit konkretem Grund — die Position wird **nicht** stillschweigend in den „nicht zugeordnet“-Bereich verschoben. Look-through ist ausserdem immer **per ISIN** kuratiert: jede ISIN hat ihr eigenes Profil; ähnlich klingende Fonds (MSCI World ≠ S&P 500, Robotik/AI ≠ breit gestreutes US-Tech, EUR-Govt ≠ Global Aggregate) werden niemals automatisch als Geschwister behandelt."
                : "To make sure a position never silently drops out of the geo / sector / single-stock aggregates, look-through coverage is enforced in three places: (1) **In the catalog itself** — every default ISIN must have a look-through profile, otherwise the catalog cannot ship at all; for alternatives and pool entries any missing slots are surfaced per bucket. (2) **In the admin add-flows** — before a new instrument is saved as default, alternative or pool entry, the app verifies live with justETF that country and sector data are available; if not, the save is blocked (a force option lets the operator take it in anyway — it will then surface under the “unmapped ETFs” alert). (3) **In the Explain tab** — when an ISIN outside the catalog is typed, the app automatically fetches its look-through profile live from justETF. On success the red “Look-through data missing” alert disappears immediately, without any reload. On failure (justETF unreachable or no geo / sector block available) a clear error message with the concrete reason is shown — the position is **not** silently moved into the “unmapped” area. Look-through is also always curated **per ISIN**: every ISIN owns its own profile; similar-sounding funds (MSCI World ≠ S&P 500, Robotics/AI ≠ broad US tech, EUR Govt ≠ Global Aggregate) are never automatically treated as siblings."}
            </p>
            <p className="text-xs text-muted-foreground">
              {de
        ...
        >
          ...
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            ...
            <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
              ...
              <li>{de
                ? "Look-Through-Profil aus dem Live-Abruf (im Browser gespeichert): Liegt für eine off-Katalog-ISIN noch kein vorab geladenes Look-Through-Profil vor, holt die App es ebenfalls live von justETF nach und merkt sich das Ergebnis im Browser, sodass es einen Reload überlebt — der kuratierte Pool bleibt unverändert, dieser Cache patcht nur die vom Nutzer getippten Off-Katalog-Zeilen ein."
                : "Look-through profile from the live lookup (stored in the browser): when no pre-loaded look-through profile exists for an off-catalog ISIN, the app fetches it live from justETF as well and remembers the result in the browser so it survives a reload — the curated pool stays untouched, this cache only patches in the user's typed off-catalog rows."}</li>
            </ul>
          </div>
