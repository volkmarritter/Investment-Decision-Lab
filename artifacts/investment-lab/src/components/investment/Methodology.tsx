        ...
        <CardContent className="space-y-3">
          ...
          <div className="rounded-md border bg-primary/5 p-3 space-y-2" data-testid="editable-overview">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              {de
                ? "Die folgenden drei Abschnitte unten enthalten Live-editierbare Eingaben. Eigene Werte werden lokal in Ihrem Browser gespeichert und überschreiben die Defaults in der gesamten App. Klappen Sie den jeweiligen Abschnitt auf, um die Felder zu sehen."
                : "The three sections listed below contain live-editable inputs. Custom values are stored locally in your browser and override the defaults across the entire app. Expand the section in question to see the fields."}
            </p>
      ...
      >
        ...
        >
          ...
          <p className="text-sm text-muted-foreground pt-2">
            {de
              ? "Vollständige Übersicht aller Allokations-Buckets der Engine, gruppiert nach Anlageklasse. Pro Bucket zeigt der Baum den aktuell hinterlegten ETF (Name + Katalog-Schlüssel). Über „Ersetzen“ können Sie eine eigene ISIN eintragen, mit den Live-Daten von justETF vergleichen und den Bucket lokal in Ihrem Browser umstellen — die Empfehlungs-Liste, die Gebühren-Berechnung (TER) und der Look-Through-Tab übernehmen den Wechsel sofort. Die Monte-Carlo-Simulation rechnet immer auf Asset-Class-Ebene (μ/σ aus den CMA-Annahmen je Bucket); die Zusammensetzung dieser Asset Classes hängt aber davon ab, ob der Look-Through-Schalter der Slot/Slots aktiv ist: ist er AUS, wird der Bucket-Schlüssel direkt auf eine Asset-Class-Mischung abgebildet (ETF-Wechsel innerhalb desselben Buckets verändern die Simulation dann nicht), ist er AN, werden die tatsächlichen Regionen-Gewichte des gewählten ETF aus den Look-Through-Daten gezogen — dann kann sich die Simulation auch innerhalb desselben Buckets verschieben (z. B. World vs. Developed vs. Emerging Markets)."
              : "Full view of every allocation bucket the engine knows about, grouped by asset class. Each leaf shows the currently selected ETF (name + catalog key). The Override button lets you type a new ISIN, compare it side-by-side with the live justETF data and swap the bucket locally in your browser — the recommendation list, the fee calculation (TER) and the look-through tab reflect the change immediately. The Monte Carlo simulation always runs at the asset-class level (μ/σ from the CMA assumptions per bucket), but the composition of those asset classes depends on the per-slot Look-Through toggle: when it is OFF, the bucket key is mapped directly to an asset-class mix (so swapping ETFs within the same bucket leaves the simulation unchanged); when it is ON, the actual regional weights of the chosen ETF are pulled from the look-through data — and the simulation can then move even within the same bucket (e.g. World vs. Developed vs. Emerging Markets)."}
          </p>
            ...
            <Accordion type="single" collapsible>
              ...
              <div className="rounded-md border bg-muted/30 p-3 text-xs leading-relaxed space-y-2">
                ...
                <div className="border-t pt-2">
                  ...
                  <div>{de ? "Sonntags 03:00 UTC" : "Sundays 03:00 UTC"}</div>
                  <div className="text-muted-foreground">
                    {de
                      ? "Aktualisierte Felder: TER (Basispunkte), Fondsgröße (Mio. EUR), Auflagedatum, Ertragsverwendung (thesaurierend / ausschüttend) und Replikationsmethode (physisch / physisch (Sampling) / synthetisch)."
                      : "Refreshed fields: TER (basis points), fund size (EUR millions), inception date, distribution policy (accumulating / distributing) and replication method (physical / physical (sampled) / synthetic)."}
                  </div>
                </div>
                <div className="border-t pt-2">
                  <div className="font-semibold mb-0.5">{de ? "2) Nächtlich — Notierungen je Börse" : "2) Nightly — per-exchange listings"}</div>
                  <div>{de ? "Täglich 02:00 UTC" : "Daily 02:00 UTC"}</div>
                  <div className="text-muted-foreground">
                    {de
                      ? "Aktualisierter Wert: Ticker-Liste je Börse (LSE / XETRA / SIX / Euronext). Bei mehreren Anteilsklassen an derselben Börse wird die Notierung in der primären Fondswährung bevorzugt (z. B. an LSE der USD-Ticker statt GBX)."
                      : "Refreshed value: per-exchange ticker list (LSE / XETRA / SIX / Euronext). When several share classes trade on the same venue, the listing in the fund's primary currency is preferred (e.g. on LSE the USD ticker rather than GBX)."}
                  </div>
                </div>
                <div className="border-t pt-2">
                  <div className="font-semibold mb-0.5">{de ? "3) Monatlich — Top-10 Holdings" : "3) Monthly — top-10 holdings"}</div>
                  <div>{de ? "Am 1. des Monats, 04:00 UTC" : "1st of month, 04:00 UTC"}</div>
                  <div className="text-muted-foreground">
                    {de
                      ? "Aktualisierter Wert: Top-10 Einzelwerte mit Gewichten je Aktien-ETF (Name, Anteil in %), plus ein Stichtag pro ETF. Nicht-Aktien-ETFs (Gold, Krypto) werden übersprungen."
                      : "Refreshed value: top-10 holdings with weights for each equity ETF (name, weight in %), plus a per-ETF as-of date. Non-equity ETFs (gold, crypto) are skipped."}
                  </div>
                </div>
                <div className="border-t pt-2">
                  ...
                  <div className="text-muted-foreground">
                    {de
                      ? "Aktualisierte Werte je Aktien-ETF: Länder-, Sektor- und Währungs-Aufteilung mit gemeinsamem Stichtag. Länder und Sektoren werden direkt aus den justETF-Tabellen geholt; sobald justETF die Tabelle zusammenklappt und einen „Show more“-Link anbietet, wird die vollständige Tabelle automatisch nachgeladen. Die Währungs-Aufteilung leitet sich aus der Länder-Aufteilung ab (justETF veröffentlicht keine eigene Währungstabelle); für währungsgesicherte Anteilsklassen bleibt die kuratierte Hedge-Währungskarte stehen."
                      : "Refreshed values per equity ETF: country, sector and currency breakdowns with a single shared as-of date. Country and sector are taken straight from the justETF tables; when justETF collapses the table behind a “Show more” link, the full breakdown is fetched automatically. The currency breakdown is derived from the country breakdown (justETF doesn't publish a separate per-ETF currency table); for currency-hedged share classes the curated hedge-currency map stays in place."}
                  </div>
                </div>
              </div>
              <p className="text-xs text-muted-foreground leading-relaxed">
                {de
                  ? "Hand-kuratiert (von keinem Snapshot überschrieben) bleiben: Default-Börse pro ETF, redaktioneller Kommentar, sowie die Währungs-Aufteilung der währungsgesicherten Anteilsklassen — dort entspricht die FX-Belastung nach Hedging der Anteilsklassen-Währung, nicht dem Länder-Mix. Bei ETFs ohne automatischen Refresh (z. B. Gold/Krypto, Anleihen-ETFs ohne Aktien-Look-Through) gilt der Stichtag Q4 2024."
                  : "Curated by hand (not overwritten by any snapshot): default exchange per ETF, editorial comment, and the currency breakdown for currency-hedged share classes — there the post-hedging FX exposure is the share-class currency, not the underlying country mix. For ETFs without an automatic refresh (e.g. gold / crypto, bond ETFs without equity look-through) the Q4 2024 reference date applies."}
              </p>
              <p className="text-xs text-muted-foreground leading-relaxed">
                {de
        ...
        >
          ...
          <div className="rounded-md border bg-muted/30 p-3 text-xs space-y-1.5" data-testid="cma-routing-note">
            ...
            <p className="text-muted-foreground">
              {de
                ? "Aktien-ETFs mit einer der gelisteten Regionen (USA, Europa, UK, Schweiz, Japan, EM) fließen direkt in die entsprechende Zeile. Zusätzlich gilt — unabhängig davon, ob die Look-Through-Analyse an oder aus ist — Kanada → USA, Irland → Europa, Asien-Pazifik ex-Japan → Japan und die manuelle Region »Other« → »Other / Residual«. Echte Sektor-/Themen-Labels (Technologie, Healthcare, Sustainability, Cybersecurity, Thematic) landen weiterhin in »Thematic Equity«. justETFs »Other«-Sammelposten landen ebenfalls in »Other / Residual«. Die vollständigen Routing-Tabellen stehen weiter unten im Abschnitt »Look-Through«."
                : "Equity ETFs whose declared region matches one of the listed rows (USA, Europe, UK, Switzerland, Japan, EM) flow directly into that row. In addition — and identically with look-through ON or OFF — Canada → USA, Ireland → Europe, Asia Pacific ex-Japan → Japan, and the manual region \"Other\" → \"Other / Residual\". Genuine sector/theme labels (Technology, Healthcare, Sustainability, Cybersecurity, Thematic) still land in \"Thematic Equity\". justETF's catch-all \"Other\" slice also lands in \"Other / Residual\". The full routing tables live further down in the \"Look-Through\" section."}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-xs text-muted-foreground">
              {de
                ? "Lass die Felder leer, um den Konsens- bzw. Engine-Wert zu nutzen. Eigene Werte werden lokal in deinem Browser gespeichert und überschreiben die Defaults überall in der App."
                : "Leave fields empty to use the consensus or engine value. Custom values are stored locally in your browser and override the defaults everywhere in the app."}
            </p>
        ...
        >
          ...
          <p className="text-sm text-muted-foreground">
            {de
              ...
              : "From the risk engine's perspective, ETFs like iShares Core MSCI Europe (IE00B4K48X80) are not a homogeneous “Europe” position: roughly 20 % is UK and 14 % is Switzerland, with the rest in continental EU. The look-through logic uses the curated index geo profiles (see Look-Through Pool below) and routes each allocation row, weighted, onto the correct country buckets — before Vol, Beta, TE, Alpha, the TE contribution and the correlation matrix are computed."}
          </p>
          ...
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            <p className="text-xs font-semibold">{de ? "Lückenlose Abdeckung (Task #238)" : "Gap-free coverage (Task #238)"}</p>
            <p className="text-xs text-muted-foreground">
              {de
                ? "Damit eine Position niemals stillschweigend aus der Geo-/Sektor-/Einzeltitel-Aggregation herausfällt, wird die Look-Through-Abdeckung an drei Stellen erzwungen: (1) **Im Katalog selbst** — jede Default-ISIN muss ein Look-Through-Profil haben, sonst lässt sich der Katalog gar nicht ausliefern; für Alternativen und Pool-Einträge wird die Lücke pro Bucket angezeigt. (2) **In den Admin-Add-Flows** — bevor ein neues Instrument als Default, Alternative oder Pool-Eintrag gespeichert wird, prüft die App live bei justETF, ob Länder- und Sektor-Daten geliefert werden; fehlen sie, wird das Speichern blockiert (eine Force-Option erlaubt es bewusst, die Position trotzdem aufzunehmen — sie erscheint dann unter „nicht zugeordnete ETFs“). (3) **Im Erklären-Tab** — wenn eine ISIN ausserhalb des Katalogs ins Portfolio gelangt, holt die App das Look-Through-Profil automatisch live von justETF nach. Das gilt für alle drei Wege, auf denen eine Off-Katalog-Zeile entsteht: direktes Tippen/Einfügen einer ISIN, der Listen-Import (Einfügen-Dialog) **und** das Laden eines gespeicherten Portfolios (aus Datei oder aus der Merkliste) — gerade letzteres ist wichtig, weil eine auf einem anderen Gerät erstellte Datei mit leerer Browser-Registry ankommt. Bei Erfolg verschwindet die rote „Look-through-Daten fehlen“-Warnung sofort, ohne Neuladen. Bei Fehlschlag (justETF nicht erreichbar oder kein Geo-/Sektor-Block verfügbar) erscheint eine deutliche Fehlermeldung mit konkretem Grund — die Position wird **nicht** stillschweigend in den „nicht zugeordnet“-Bereich verschoben. Bereits durch Katalog/Overrides oder die Browser-Registry abgedeckte ISINs werden dabei nie erneut abgerufen. Look-through ist ausserdem immer **per ISIN** kuratiert: jede ISIN hat ihr eigenes Profil; ähnlich klingende Fonds (MSCI World ≠ S&P 500, Robotik/AI ≠ breit gestreutes US-Tech, EUR-Govt ≠ Global Aggregate) werden niemals automatisch als Geschwister behandelt."
                : "To make sure a position never silently drops out of the geo / sector / single-stock aggregates, look-through coverage is enforced in three places: (1) **In the catalog itself** — every default ISIN must have a look-through profile, otherwise the catalog cannot ship at all; for alternatives and pool entries any missing slots are surfaced per bucket. (2) **In the admin add-flows** — before a new instrument is saved as default, alternative or pool entry, the app verifies live with justETF that country and sector data are available; if not, the save is blocked (a force option lets the operator take it in anyway — it will then surface under the “unmapped ETFs” alert). (3) **In the Explain tab** — when an ISIN outside the catalog enters the portfolio, the app automatically fetches its look-through profile live from justETF. This applies to all three ways an off-catalog row can appear: typing or pasting an ISIN directly, the list import (paste dialog) **and** loading a saved portfolio (from a file or from the bookmarks list) — the last case matters most, because a file created on another device arrives with an empty browser registry. On success the red “Look-through data missing” alert disappears immediately, without any reload. On failure (justETF unreachable or no geo / sector block available) a clear error message with the concrete reason is shown — the position is **not** silently moved into the “unmapped” area. ISINs already covered by the catalog/overrides or the browser registry are never re-fetched. Look-through is also always curated **per ISIN**: every ISIN owns its own profile; similar-sounding funds (MSCI World ≠ S&P 500, Robotics/AI ≠ broad US tech, EUR Govt ≠ Global Aggregate) are never automatically treated as siblings."}
            </p>
            <p className="text-xs text-muted-foreground">
              {de
                ? "Wo trotzdem ein Profil fehlt — etwa bei Anleihen-, Geldmarkt- und Gold-ETCs, für die justETF keine Geo-/Sektor-Tabellen veröffentlicht — listet die Look-Through-Karte in Build und Erklären die betroffenen Positionen (ISIN, Name, Gewicht) namentlich auf. Eine permanente Warnung über der (eingeklappten) Karte stellt sicher, dass die fehlenden Positionen immer sichtbar sind, auch ohne die Karte aufzuklappen."
                : "Where a profile is still missing — e.g. for bond, money-market and gold ETCs that justETF doesn't publish geo / sector tables for — the look-through card in Build and Explain lists the affected positions (ISIN, name, weight) by name. A permanent warning above the (collapsed) card makes sure the missing positions are always visible, even without expanding it."}
            </p>
          </div>
        </Section>
        ...
        >
          ...
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            ...
            <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
              <li>{de
                ? "Kuratierter Katalog (sofort verfügbar): Ist die ISIN bereits Teil des kuratierten Katalogs, gelten die geprüften Felder (Name, Währung, TER, Domizil, Replikation, Ausschüttung, Fondsvolumen, Auflagedatum) in der Vorschau als Wahrheit."
                : "Curated catalog (instant): if the ISIN is already in the curated catalog, the verified fields (name, currency, TER, domicile, replication, distribution, fund size, inception) are treated as truth in the preview."}</li>
              <li>{de
                ? "Vorab geladene Look-Through-Profile (sofort verfügbar): Für die im Katalog geführten ETFs sind Länder-, Sektor- und Top-Holdings-Aufteilung bereits im App-Bundle enthalten und werden zusammen mit ihrem Stichtag angezeigt."
                : "Pre-loaded look-through profiles (instant): for ETFs in the curated catalog the country, sector and top-holdings breakdowns are already bundled with the app and are shown together with their as-of date."}</li>
              <li>{de
                ? "Live-Abruf von justETF (kurz nach dem Tippen): Für ISINs ausserhalb des Katalogs werden die Stammdaten kurz nach Tippstopp live von justETF nachgeladen. Antworten werden im Browser zwischengespeichert; ein moderates Anfrage-Limit pro Nutzer schützt justETF vor Überlast."
                : "Live justETF lookup (shortly after typing stops): for ISINs outside the catalog the master data is fetched live from justETF a moment after typing pauses. Responses are cached in the browser; a moderate per-user request limit protects justETF from overload."}</li>
              <li>{de
                ? "Look-Through-Profil aus dem Live-Abruf (im Browser gespeichert): Liegt für eine off-Katalog-ISIN noch kein vorab geladenes Look-Through-Profil vor, holt die App es ebenfalls live von justETF nach und merkt sich das Ergebnis im Browser, sodass es einen Reload überlebt — der kuratierte Pool bleibt unverändert, dieser Cache patcht nur die Off-Katalog-Zeilen des Nutzers ein (egal ob getippt, per Liste importiert oder aus einem gespeicherten Portfolio geladen)."
                : "Look-through profile from the live lookup (stored in the browser): when no pre-loaded look-through profile exists for an off-catalog ISIN, the app fetches it live from justETF as well and remembers the result in the browser so it survives a reload — the curated pool stays untouched, this cache only patches in the user's off-catalog rows (whether typed, list-imported, or restored from a saved portfolio)."}</li>
            </ul>
          </div>
          ...
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            ...
            <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
              ...
              <li>{de
                ? "Look-Through-Banner: Sobald für die ISIN ein Profil verfügbar ist — entweder aus dem vorab geladenen Pool oder aus dem Live-Abruf bei justETF — erscheint ein grünes Banner („Look-Through verfügbar“) mit der Anzahl Regionen / Sektoren / Top-Holdings und dem Stichtag. Der Banner wird sofort grün, sobald der Live-Abruf für die getippte ISIN erfolgreich war, ohne dass die Eingabe noch einmal angefasst werden muss. Die bernsteinfarbene 0-%-Warnung erscheint nur dann, wenn überhaupt kein Profil verfügbar ist (weder vorab geladen noch live abrufbar) — z. B. typische CH-domizilierte Fonds ohne justETF-Geo-Tabelle."
                : "Look-through banner: as soon as a profile is available for the ISIN — either from the pre-loaded pool or from the live justETF lookup — a green banner (“look-through available”) shows the region / sector / top-holding counts and the as-of date. The banner turns green the instant the live lookup succeeds for the typed ISIN, with no need to re-touch the input. The amber 0 % warning only fires when no profile is available at all (neither pre-loaded nor live) — e.g. typical CH-domiciled funds for which justETF doesn't publish a country table."}</li>
              <li>{de
              ...
              <li>{de
                ? "Automatische Klassifizierung von Asset-Klasse und Region: Sobald die Stammdaten von justETF eintreffen, leitet die App aus Name und Währung eine plausible Asset-Klasse / Region ab und trägt sie ein — aber nur, wenn die Zeile noch die frischen Standardwerte (Equity / Global) trägt. Eine zweisprachige Hinweiszeile („automatisch erkannt — überschreibbar“) macht das Vorgehen sichtbar; sobald der Nutzer eine der beiden Auswahllisten anpasst, verschwindet der Hinweis und die App schreibt nichts mehr automatisch in diese Zeile. Die Heuristik nutzt dieselbe Logik wie der Picker für nicht zugeordnete Katalog-Instrumente."
                : "Automatic asset-class & region classification: as soon as the master data arrives from justETF, the app derives a plausible asset class / region from name and currency and fills them in — but only while the row still carries the fresh defaults (Equity / Global). A bilingual hint (“auto-detected — overridable”) makes the action visible; the moment the operator adjusts either dropdown the hint disappears and the app stops writing anything automatic to that row. The heuristic reuses the same logic as the unassigned-catalog-instrument picker."}</li>
              <li>{de
                ? "Stiller Look-Through-Toast bei automatisch klassifizierten Zeilen: Schlägt der Look-Through-Abruf für eine off-Katalog-ISIN fehl, wird der rote Toast bewusst um ~1,5 s verzögert und unterdrückt, sobald die Stammdaten zwischenzeitlich eine automatische Klassifizierung getriggert haben — die bernsteinfarbene 0-%-Hinweiszeile in der Zeile bleibt sichtbar und genügt. Nur wirklich nicht klassifizierbare Zeilen lösen den lauten Toast weiterhin aus."
                : "Quieter look-through toast when the row was auto-classified: when the look-through lookup fails for an off-catalog ISIN, the red toast is intentionally deferred by ~1.5 s and muted if the master-data scrape has meanwhile triggered an auto-classification — the in-row amber 0 % banner stays visible and is enough. Only genuinely un-classifiable rows still raise the loud toast."}</li>
              <li>{de
          ...
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              {de
                ? "Sobald die manuelle Position im Portfolio steht, werden Währung und (sofern vom Nutzer gesetzt) TER aus den Eingabefeldern übernommen (die Schaltfläche „Werte übernehmen“ befüllt diese vor dem Speichern). Für die TER der Zeile gilt eine dreistufige Fallback-Kette, damit der Gebühren-Schätzer für off-Katalog-ETFs nicht still auf 0,0 bps fällt: (1) der vom Nutzer eingetragene TER-Wert hat Vorrang, (2) andernfalls wird der zuletzt aus justETF gescrapte TER für diese ISIN aus dem Tab-Cache der Vorschau verwendet (auch ohne Klick auf „Werte übernehmen“), (3) andernfalls greift der Asset-Klassen-Standardwert aus `getETFTer(assetClass, region)` — derselbe Wert, den der Schätzer ohnehin für unbekannte Buckets ansetzt. Im Gebühren-Schätzer kennzeichnet ein kleines Badge je Zeile, welcher dieser drei Schritte den Wert geliefert hat („Operator“, „justETF“ oder „Standard“); Katalog-Zeilen bleiben unverändert ohne Badge. Replikation und Domizil bleiben leer und die Ertragsverwendung wird auf „Thesaurierend“ gesetzt — diese Felder werden bewusst nicht ein zweites Mal von justETF nachgeladen. Im Kommentar erscheint bei einem Look-Through-Treffer der Hinweis „Manuell erfasst — Pool-Look-Through aus justETF (Stand: JJJJ-MM-TT)“; andernfalls steht dort „Manuell erfasst — keine Katalog-Look-Through-Daten verfügbar.“ Wichtig: Die eigentliche Look-Through-Berechnung (Geo, Sektoren, Top-Holdings, Home-Bias) läuft unabhängig davon weiter — der Pool-Treffer ist also bereits aktiv, der Kommentar macht ihn nur sichtbar."
                : "Once the manual position is in the portfolio, currency and (when the user set it) TER are taken from the input fields (the “Use these values” button populates them before saving). The row's TER follows a three-step fallback chain so the Fee Estimator doesn't silently land at 0.0 bps for off-catalog ETFs: (1) an operator-entered TER wins, (2) otherwise the most recent justETF-scraped TER for that ISIN is read from the in-tab preview cache (even without clicking “Use these values”), (3) otherwise the asset-class default from `getETFTer(assetClass, region)` is used — the same value the estimator falls back to for unknown buckets. The Fee Estimator labels each row with a small badge showing which of those three steps produced the value (“operator”, “justETF” or “default”); catalog rows continue to render without a badge. Replication and domicile are left blank, and distribution is set to “Accumulating” — these fields are intentionally not fetched a second time from justETF. The comment line shows “Manually entered — pool look-through from justETF (as of YYYY-MM-DD)” when a look-through profile is available; otherwise it reads “Manually entered — no catalog look-through data available.” Important: the actual look-through math (geo, sectors, top-holdings, home-bias) runs independently regardless — the pool hit is already active, the comment just makes it visible."}
            </p>
          </div>
          ...
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              {de
                ? "Wenn beim Tippen mehrere Vorschauen unterwegs sind, wird eine späte Antwort von einer früheren ISIN verworfen — sie kann niemals in eine Zeile geschrieben werden, deren ISIN inzwischen geändert wurde. Strukturell fehlerhafte Antworten werden als klare Fehlermeldung in der Sprache des Nutzers angezeigt; Netzwerk-, Timeout- und Rate-Limit-Fehler werden mit der ursprünglichen, vom Server gelieferten Meldung direkt unter dem Eingabefeld angezeigt — kurz genug, um zu erkennen, ob es sich um ein Rate-Limit, ein Timeout oder ein echtes Datenproblem handelt."
                : "When several previews are in flight while the user is typing, a late response from an earlier ISIN is discarded — it can never be written onto a row whose ISIN has since changed. Structurally malformed responses are surfaced as a clear error message in the user's language; network, timeout and rate-limit errors are shown inline below the input field using the server's original message — short enough to tell rate-limit, timeout and a genuine data problem apart."}
            </p>
          </div>
        </Section>
        ...
        <Section value="hedging" icon={<Coins className="h-4 w-4" />} title={de ? "Währungs-Hedging — was der Schalter wirklich tut" : "Currency Hedging — what the toggle actually does"}>
          ...
          <ul className="text-sm space-y-2 list-disc pl-5">
            ...
            <li>
              {de
                ? <><span className="font-medium text-foreground">Synthetik-US-Gate (Build &amp; Explain):</span> hedged Anteilsklassen deaktivieren den Synthetik-US-Carve-out. Begründung: hedged-synthetische S&amp;P-500-Share-Classes sind in der UCITS-Realität atypisch; den vollen WHT-Benefit dafür anzurechnen würde die Erwartungsrendite überzeichnen. Monte-Carlo-Pfade und analytische Kennzahlen halten sich gemeinsam an dasselbe Gate.</>
                : <><span className="font-medium text-foreground">Synthetic-US gate (Build &amp; Explain):</span> hedged share classes disable the synthetic-US carve-out. Reason: hedged synthetic S&amp;P 500 share classes are atypical in the real UCITS catalog; granting the full WHT benefit there would overstate expected return. Monte Carlo paths and analytical metrics both honour this gate together.</>}
            </li>
            <li>
              {de
            ...
            <li>
              {de
                ? <><span className="font-medium text-foreground">„Fremdwährungs-Anleihen absichern" (Bond-only Hedge, Default AN):</span> ein zweiter, präziserer Schalter neben der vollen Währungsabsicherung. Wenn AN, der volle Hedging-Schalter AUS und die Basiswährung ≠ USD ist, gilt das Hedging-Setup ausschließlich für den Fixed-Income-Sleeve — d. h. (a) die Bucket→ETF-Logik im Build routet nur Anleihen in die hedged Anteilsklasse (FixedIncome-Global-EUR/CHF/GBP), Aktien bleiben unhedged; (b) der Fee-Estimator schlägt die +15 Bp ausschließlich auf Fixed Income auf, nicht auf Equity/Real Estate; (c) Monte Carlo nimmt den −1 Pp-σ-Schnitt (Boden 0.01) ausschließlich auf Anleihen-Buckets vor. Aktien-Sigma und Aktien-Korrelationen bleiben unverändert. Der volle Hedging-Schalter subsumiert diesen — sobald „Volle Währungsabsicherung" AN ist, ist Bond-only redundant und im UI als „bereits abgedeckt" ausgegraut. Bei Basiswährung USD ist der Schalter komplett ausgeblendet (keine Fremdwährungs-Anleihen-Exposure zu hedgen). Stored-portfolio-files ohne dieses Feld werden als AN interpretiert (Default).</>
                : <><span className="font-medium text-foreground">"Hedge foreign-currency bonds" (bond-only hedge, default ON):</span> a second, more precise toggle alongside the full currency hedge. When ON, the full-hedge toggle is OFF, and the base currency is ≠ USD, the hedging setup applies only to the Fixed Income sleeve — i.e. (a) the Build bucket→ETF mapping routes only bonds to the hedged share class (FixedIncome-Global-EUR/CHF/GBP), equities stay unhedged; (b) the fee estimator adds the +15 bps only to Fixed Income, not Equity/Real Estate; (c) Monte Carlo applies the −1 pp σ cut (floor 0.01) only to Fixed Income buckets. Equity sigma and equity correlations are untouched. The full-hedge toggle subsumes this one — as soon as "Currency Hedging" is ON, bond-only is redundant and shown as "already covered" / disabled in the UI. For USD-base portfolios the toggle is hidden entirely (no foreign-currency bond exposure to hedge). Stored portfolio files that pre-date this flag are loaded as ON (default).</>}
            </li>
          </ul>
          <p className="text-sm text-muted-foreground pt-2">
          ...
          <ul className="text-sm space-y-2 list-disc pl-5">
            <li>
              {de
                ? <><span className="font-medium text-foreground">Erkennung:</span> ein ETF wird in der Währungsübersicht als hedged behandelt, wenn entweder (a) seine ISIN auf der kuratierten Liste hedged ETFs steht oder (b) sein Name das Wort „Hedged“ enthält. Das ist <span className="font-medium text-foreground">unabhängig vom globalen Hedging-Schalter</span> — es ist eine Eigenschaft des ETFs selbst.</>
                : <><span className="font-medium text-foreground">Detection:</span> an ETF is treated as hedged in the Currency Overview when either (a) its ISIN is on the curated list of hedged ETFs, or (b) its name contains the word "Hedged". This is <span className="font-medium text-foreground">independent of the global hedging toggle</span> — it's a property of the ETF itself.</>}
            </li>
            <li>
              {de
                ? <><span className="font-medium text-foreground">Zuordnung:</span> das volle Gewicht des ETFs wird der <span className="font-medium text-foreground">hedged-Sleeve seiner Anteilsklassen-Währung</span> zugeschlagen (mit Fallback auf die Basiswährung, falls die Anteilsklassen-Währung fehlt). Die Underlying-Währungen werden für hedged ETFs bewusst ignoriert — kein Look-Through wird konsultiert.</>
                : <><span className="font-medium text-foreground">Assignment:</span> the ETF's full weight is added to the <span className="font-medium text-foreground">hedged sleeve of its share-class currency</span> (falling back to the base currency if the share-class currency is missing). The underlying holdings' currencies are deliberately ignored for hedged ETFs — no look-through is consulted.</>}
            </li>
            <li>
              {de
        ...
        <Section value="wht" icon={<Coins className="h-4 w-4" />} title={de ? "Quellensteuer-Drag" : "Withholding-Tax Drag"} version={sectionVersionLong("wht")}>
          ...
          >
            ...
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              {de
                ? <><span className="font-semibold text-foreground">Hinweis zur Statik:</span> Die Drag-Werte sind im Code als feste Konstanten hinterlegt — sie ziehen NICHT live mit den tagesaktuellen Dividenden-Renditen mit. Begründung: die annualisierte Yield einer Indexregion bewegt sich über Quartale nur in der Größenordnung von 20–50 bps, während die Streuung zwischen LTCMA-Anbietern beim Yield-Input deutlich größer ist. Eine Live-Berechnung würde Genauigkeit nur vortäuschen.</>
                : <><span className="font-semibold text-foreground">Static-by-design:</span> drag values are kept as fixed constants in code — they do NOT track today's published dividend yields in real time. Rationale: an index region's annualised yield moves only ~20–50 bps quarter-on-quarter, while LTCMA-provider dispersion on the yield input is materially larger. A "live" recompute would be false precision.</>}
            </p>
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              {de
          ...
          <div className="rounded-md border border-border bg-muted/30 p-3 text-xs">
            ...
            <p className="mt-2 text-muted-foreground">
              {de
                ? "Quelle: zentrale, im Code versionierte WHT-Drag-Konstante; dort werden die Sätze und Quellen-Annahmen gepflegt."
                : "Source: a single WHT-drag constant kept in code, where rates and source assumptions are versioned."}
            </p>
          </div>
          <p className="text-xs text-muted-foreground">
            {de
        ...
        <Section value="mc" icon={<Calculator className="h-4 w-4" />} title={de ? "Monte-Carlo-Simulation" : "Monte Carlo Simulation"} version={sectionVersionLong("mc")}>
          <ul className="text-sm space-y-2 list-disc pl-5">
            ...
            <li>{de ? "Korrelation & Schock-Ebene: das Portfolio wird vorab zu einem einzigen μ und σₚ kollabiert — μ ist das gewichtete Mittel der Klassen-μ, σₚ wird aus der vollständigen Kovarianzmatrix berechnet (Formel im Abschnitt „Formeln“). Anschließend wird pro Jahr eine einzige Gauß-Ziehung auf Portfolio-Ebene gezogen und auf das Portfolio als Ganzes angewendet — NICHT eine Ziehung pro Anlageklasse, die anschließend gewichtet wird. Die Diversifikation steckt also bereits in σₚ über die Kovarianzmatrix; sie entsteht nicht erst durch unabhängige Klassen-Ziehungen im Pfad." : "Correlation & shock level: the portfolio is collapsed up front into a single μ and σₚ — μ is the weighted average of the per-class μs, σₚ is computed from the full covariance matrix (formula in the \"Formulas\" section). Then, per year, a single Gaussian shock is drawn at the portfolio level and applied to the portfolio as a whole — NOT one draw per asset class that is then weighted. Diversification is therefore already baked into σₚ via the covariance matrix; it does not emerge from independent per-class draws inside a path."}</li>
            <li>{de ? "Praktische Konsequenz: die Engine erzeugt keine Pfade auf Anlageklassen-Ebene. Effekte, die voneinander abweichende Sleeve-Pfade voraussetzen — etwa Rebalancing-Dynamik oder Drawdowns einzelner Sleeves — werden daher nicht modelliert; ausgewiesen werden ausschließlich gemischte Portfolio-Ergebnisse." : "Practical consequence: the engine does not produce per-asset-class paths. Anything that would require sleeves to diverge within a path — e.g. rebalancing dynamics or sleeve-level drawdowns — is therefore not modelled; only blended portfolio outcomes are reported."}</li>
