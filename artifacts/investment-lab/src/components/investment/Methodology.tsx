      ...
      >
        ...
        <Section value="lookthrough" icon={<Layers className="h-4 w-4" />} title={de ? "Look-Through-Routing" : "Look-Through Routing"}>
          ...
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              {de
                ? "Damit eine Position niemals stillschweigend aus der Geo-/Sektor-/Einzeltitel-Aggregation herausfällt, wird die Look-Through-Abdeckung an drei Stellen erzwungen: (1) der Katalog selbst — `validateLookthroughCoverage()` in src/lib/etfs.ts liefert pro Bucket die Slots (Default · Alternative · Pool), die kein Profil haben; eine Vitest-Regression sperrt fehlende Default-Profile vollständig (jede Default-ISIN muss abgedeckt sein, weil sie die meistgezeigte Picker-Option ist). (2) die Admin-Add-Flows — `POST /admin/buckets/:key/alternatives`, `PUT /admin/buckets/:key/default`, `POST /admin/buckets/:key/pool` und der Alternative-Pfad in `POST /admin/instruments` rufen vor dem Schreiben `scrapeLookthrough(isin)` auf und antworten mit `422 lookthrough_incomplete`, falls keine `geo + sector`-Tabelle geliefert wird; ein optionales `force: true` im Body überschreibt die Sperre und führt die Position bewusst als „nicht zugeordnet\u201C in der Look-Through-Karte. (3) der Erklären-Tab — beim Tippen einer ISIN ausserhalb des Katalogs ruft `setManualIsin` den öffentlichen `GET /api/lookthrough-scrape/:isin`-Endpoint (gleiche Per-IP-Rate-Limiter, Cache und In-Flight-Dedup wie `/etf-preview`; **read-only**: dieser Pfad schreibt nichts in den Pool, weil der öffentliche Endpoint ungeschützt ist und sonst eine Repository-Mutationen-Lücke wäre — Pool-Eintragungen bleiben dem Admin-Flow vorbehalten). Bei Erfolg schreibt der Tab das Ergebnis in eine prozessinterne `RUNTIME_PROFILES`-Registry (siehe `registerRuntimeLookthroughProfile` in src/lib/lookthrough.ts) und löst eine Neuberechnung des `portfolio`-Memos aus — die destruktive „Look-through-Daten fehlen\u201C-Warnung verschwindet sofort für die betroffene Zeile, ohne Neuladen. **Bei Fehlschlag** (justETF nicht erreichbar, Rate-Limit, oder kein Geo-/Sektor-Block lieferbar) erscheint eine destruktive Toast-Meldung mit konkretem Grund — die Position landet **nicht** stillschweigend im \u201Enicht zugeordnet\u201C-Bereich. Look-through ist außerdem immer **per-ISIN**: jede Katalog-ISIN besitzt ihren **eigenen** Profil-Eintrag in `PROFILES` (kuratierte Primärwerte), `SHARED_BASKET_PROFILES` (echte Index- bzw. Anteilsklassen-Äquivalente, z. B. mehrere S&P-500-Tracker) oder `DISTINCT_PROFILES` (Funds, die fälschlich als Geschwister gelten könnten — MSCI World ≠ S&P 500, Robotik/AI ≠ breiter US-Tech, EUR-Govt ≠ Global Aggregate). Es gibt **keinen** ALIAS-Lookup, der einen Geschwister-Treffer untergeschoben hätte."
                : "To make sure a position never silently drops out of the geo / sector / single-stock aggregates, look-through coverage is enforced in three places: (1) the catalog itself — `validateLookthroughCoverage()` in src/lib/etfs.ts returns the per-bucket slots (default · alternative · pool) without a profile; a Vitest regression locks down missing defaults completely (every default ISIN must be covered because it is the most-shown picker option). (2) the admin add-flows — `POST /admin/buckets/:key/alternatives`, `PUT /admin/buckets/:key/default`, `POST /admin/buckets/:key/pool` and the alternative path in `POST /admin/instruments` call `scrapeLookthrough(isin)` before persisting and return `422 lookthrough_incomplete` if `geo + sector` cannot be obtained; an optional `force: true` body flag overrides the gate and lets the operator save the entry knowing it will surface under the “unmapped ETFs” alert. (3) the Explain tab — when an off-catalog ISIN is typed, `setManualIsin` calls the public `GET /api/lookthrough-scrape/:isin` endpoint (same per-IP rate limiter, cache and in-flight dedup as `/etf-preview`; **read-only**: this path does not write into the pool because the public endpoint is unauthenticated and would otherwise be a repository-mutation hole — pool persistence stays admin-only). On success the tab registers the result into a process-local `RUNTIME_PROFILES` registry (see `registerRuntimeLookthroughProfile` in src/lib/lookthrough.ts) and bumps a state counter so the `portfolio` memo recomputes — the destructive “Look-through data missing” alert clears immediately for that row without any reload. **On failure** (justETF unreachable, rate-limited, or no geo/sector block returned) a destructive toast surfaces the concrete reason — the position is **not** silently parked under the “unmapped” alert. Look-through is also always **per-ISIN**: every catalog ISIN owns its own profile entry in `PROFILES` (curated primaries), `SHARED_BASKET_PROFILES` (true index / share-class equivalents, e.g. multiple S&P 500 trackers) or `DISTINCT_PROFILES` (funds that would be wrongly treated as siblings — MSCI World ≠ S&P 500, Robotics/AI ≠ broad US tech, EUR Govt ≠ Global Aggregate). There is **no** ALIAS lookup that would silently substitute a sibling profile."}
            </p>
            <p className="text-xs text-muted-foreground">
              {de
                ? "Wo trotzdem ein Profil fehlt — etwa bei Anleihen-, Geldmarkt- und Gold-ETCs, für die justETF keine Geo-/Sektor-Tabellen veröffentlicht — meldet die Build-/Erklären-Look-Through-Karte das laut über das strukturierte Feld `unmappedEtfs[]` (ISIN, Name, Gewicht) im Ergebnis von `buildLookthrough`. Die Karte zeigt eine permanente, destruktive Warnung über dem (eingeklappten) Inhalt — bedienerseitig sind die fehlenden Positionen damit immer sichtbar, auch ohne die Karte aufzuklappen."
                : "Where a profile is still missing — e.g. for bond, money-market and gold ETCs that justETF doesn't publish geo / sector tables for — the Build / Explain look-through card reports it loudly via the structured `unmappedEtfs[]` field (ISIN, name, weight) on the `buildLookthrough` result. The card renders a permanent, destructive alert above the (collapsed) body — operators always see the missing positions even without expanding the card."}
            </p>
          </div>
        </Section>
        ...
        >
          ...
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            ...
            <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
              ...
              <li>{de
                ? "Off-Katalog Look-Through-Cache (browser-lokal, persistent): wenn die ISIN nicht im kuratierten Pool liegt, ruft Erklären zusätzlich GET /api/lookthrough-scrape/:isin auf, hinterlegt das Ergebnis in einem prozess-lokalen Runtime-Profil-Register UND in window.localStorage (Key investment-lab.lookthrough.runtime.v1). Dadurch übersteht das Profil einen Reload im selben Browser, ohne dass die öffentliche Route schreibend in die kuratierten Pool-Daten eingreifen müsste — der kuratierte Pool bleibt die Single Source of Truth, der Cache patcht nur nutzergetippte Off-Katalog-Zeilen ein."
                : "Off-catalog look-through cache (per-browser, persistent): when the ISIN is not in the curated pool, Explain additionally calls GET /api/lookthrough-scrape/:isin, stores the result in a process-local runtime-profile registry AND in window.localStorage (key investment-lab.lookthrough.runtime.v1). That lets the profile survive a reload in the same browser without the public route ever having to write into the curated pool data — the curated pool remains the single source of truth, the cache only patches in user-typed off-catalog rows."}</li>
            </ul>
          </div>
          ...
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            ...
            <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
              ...
              <li>{de
                ? "Look-Through-Banner: grünes Banner („Look-Through verfügbar“) mit Anzahl Regionen / Sektoren / Top-Holdings + Stand-Datum, sobald für die ISIN ein Profil bereitsteht — entweder aus dem kuratierten Pool (`PROFILES` in src/lib/lookthrough.ts, gespeist aus `lookthrough.overrides.json`) ODER aus der RUNTIME_PROFILES-Registry, in die der Live-Abruf von `GET /api/lookthrough-scrape/:isin` für off-Katalog-ISINs zurückschreibt (siehe vorherige Section). Die Vorschau abonniert RUNTIME_PROFILES via `useSyncExternalStore` (`subscribeRuntimeLookthrough` + `getRuntimeLookthroughVersion`), sodass das Banner sofort grün wird, wenn der Live-Abruf für die getippte ISIN erfolgreich resolved — auch wenn die ISIN-Eingabe selbst unverändert bleibt. Die bernsteinfarbene 0-%-Warnung erscheint deshalb nur dann, wenn KEIN Profil verfügbar ist (weder Pool noch Live-Abruf liefern Geo-/Sektor-Daten) — z. B. typische CH-domizilierte Fonds ohne justETF-Geo-Tabelle."
                : "Look-through banner: green banner (“look-through available”) with region / sector / top-holding counts + as-of date as soon as a profile exists for the ISIN — either from the curated pool (`PROFILES` in src/lib/lookthrough.ts, fed by `lookthrough.overrides.json`) OR from the RUNTIME_PROFILES registry that the live `GET /api/lookthrough-scrape/:isin` lookup writes into for off-catalog ISINs (see the previous section). The preview subscribes to RUNTIME_PROFILES via `useSyncExternalStore` (`subscribeRuntimeLookthrough` + `getRuntimeLookthroughVersion`) so the banner flips to green the moment the live lookup resolves for the typed ISIN — even though the typed ISIN itself didn't change. The amber 0 % warning therefore only fires when NO profile is available (neither the pool nor the live lookup yielded geo / sector data) — e.g. typical CH-domiciled funds for which justETF doesn't publish a country table."}</li>
              <li>{de
