...
### 2026-06 (explain-file-load-lookthrough — Task #317)
- **Loading a saved portfolio now auto-fetches look-through data.** Previously only the paste-import flow fanned out justETF look-through scrapes for off-catalog ISINs. Loading a saved `.json` portfolio file (or picking a saved item from the bookmarks list) restored the workspace state but did **not** trigger the scrapes, so the Geo / Sector / Top-Holdings charts stayed empty (red "Look-through data missing" alert) until the user re-touched each ISIN — worst on a file created on another device/browser where the runtime registry starts empty.
- **What changed.** `loadWorkspace` (invoked by both file upload and selecting a saved portfolio, since `SavedExplainPortfoliosUI` routes both through `onLoadPortfolio` → `loadWorkspace`) now fans out look-through scrapes for the restored off-catalog rows that lack a profile, with per-row pending spinners and Geo/Sector/Top-Holdings filling in as results arrive — same UX as the paste-import flow.
- **Shared helper.** Extracted the scrape-trigger logic (previously inline in `replaceWithImportedRows`) into a single `fanOutLookthroughScrapes(rows)` helper so the import and file-load call sites stay in sync. It reuses `triggerImportLookthroughScrapes` + `lookthroughProfileFor` dedup guard + `handleManualScrapeResult` + the pending-spinner set, with `deferToast: false` / `allowMute: false` (the classification is baked into the saved/imported `manualMeta`, not the parallel Stammdaten auto-classifier).
- **Dedup preserved.** ISINs already covered by the bundled catalog/overrides or the runtime registry are NOT re-scraped (the `lookthroughProfileFor` guard), so loading a catalog-only file triggers nothing.
- **Methodology updated (DE + EN).** The "gap-free coverage" point (3) and the "data sources" live-lookup bullet now state the live justETF fetch fires for all three off-catalog entry paths — typed, list-import, and saved-portfolio load.
- **Tests.** Added e2e `tests/e2e/explain-file-load-lookthrough-scrape.spec.ts` (loads a file with an off-catalog ISIN, asserts the mocked scrape fires and the Geo card's Japan cell climbs 0% → ~30% with no re-typing). `pnpm run typecheck` + `test:engine` + `test:components` + the `explain` e2e group green.

### 2026-05 (pptx-export-deck-polish — Task #314)
- **PPTX export bug fix.** The combo chart on the Monte Carlo slide caused a hard download failure under pptxgenjs 4.0.1. The 3-arg form `slide.addChart(type, multi, opts)` is broken in that release — `tmpOpt = data || opt` consumes the empty `data` placeholder `[]` as truthy and discards the real options object. Switched to the 2-arg form via an `as unknown as ...` cast (`(slide.addChart as ...)([multi], opts)`), with an inline comment explaining the trap.
- **Visual polish to match the in-tab deck.** Rewrote `src/lib/exportReportPptx.ts` to mirror the typography and palette of the `portfolio-report-deck` artifact while keeping every element fully editable in PowerPoint. Design tokens: navy primary `#0B2545`, gold accent `#C9A14A`, warm paper background `#FBF7EF`, cream cards `#F3ECDC`, hairline rule `#D6CFC1`, ink `#14202E`, muted `#6B7280`. Bucket palette: equity `#1A3A5C`, bonds `#3D7A5C`, real estate `#A86F3D`, commodities `#8C5A30`, crypto `#6B4F8A`, cash `#8A8A8A`. Fonts: Georgia (display, Fraunces stand-in), Calibri (body, IBM Plex Sans stand-in), Consolas (mono, IBM Plex Mono stand-in) — chosen because they are PowerPoint-safe defaults that travel reliably.
- **New slide chrome.** Section pages share a consistent header (top-left mono uppercase "NN · Section" in gold + top-right page number, big serif headline, optional sans-serif subhead, gold accent rule at the bottom). The Cover and Disclaimer slides invert this on a navy background with a gold diamond glyph, brand mark, white serif headline (with gold word-break on the cover), and a four-chip metadata strip / report-ID footer. Reusable helpers added: `paperSlide`, `sectionHeader`, `accentRule`, `metricCard`, `chip`, `headerCell`, `bodyCell`.
- **Per-slide changes.**
  - **Cover** — navy bg with light vertical pinstripe pattern, gold diamond + "INVESTMENT DECISION LAB" wordmark, top-right report ID / issue date, oversized "Portfolio / Report." serif title (white + gold), profile one-liner in italics, and a four-chip strip (Base currency, Correlation regime, Generated on, Prepared for).
  - **TOC** — typographic list with mono section numbers in gold, serif titles in navy, mono page numbers in muted ink, hairline separators between rows.
  - **Profile** — five chips across the top (currency, risk, horizon, target equity, # of ETFs), with the two "interesting" ones rendered in the accent variant. Engine toggles render as label + pill (gold-on-cream when on, grey when off) instead of plain key/value rows. Profile-notes column on the right.
  - **Key metrics** — five accent/cream metric cards across the top (each with mono uppercase label + big serif value + muted-sans sub), three insight cards across the bottom.
  - **Allocation** — paper-coloured table with gold mono headers and hairline row separators on the left; below the table, a colour-block strip shows per-asset-class totals with the matching bucket colour, mono label, big serif percentage. Doughnut chart on the right uses the bucket palette, sits on a paper-coloured `chartArea`, has a hole size of 62, white mono data labels and a sans bottom legend.
  - **ETF implementation** — full-width table with mono ISIN / ticker / weight / TER columns and a sans navy name column. `autoPage: true` with header repeat so long catalogs roll cleanly onto extra slides.
  - **Top equity holdings** — narrower paper table; name in navy serif weight, source in muted sans, % columns mono right-aligned.
  - **Monte Carlo** — combo chart on the left (gold-tinted P10–P50 / P50–P90 stacked-area bands on a cream plot area with the invisible P10 base in paper colour so it disappears, plus a 2.5-pt navy P50 line) and a 3×2 grid of metric cards on the right (P50/P10/P90 final + expected return / vol / P-loss). Axis labels and titles use mono font in muted ink.
  - **Fees** — three big stat cards across the top (Year 1 fee accented, total drag, drag p.a.), plus a clean breakdown table below with mono numeric columns.
  - **Methodology & disclaimer** — navy bg mirroring the cover (gold diamond, mono section marker, white serif "How to read this." headline), full disclaimer paragraph in body sans, report ID footer in italic muted mono.
- **Behaviour unchanged.** Snapshot shape, fonts/colors are pure presentation, no engine / catalog / MC / look-through / persistence logic changes. Methodology page not affected.
- **Tests.** `pnpm run typecheck` and `pnpm --filter @workspace/investment-lab run test:components` green; the combo-chart bug-fix branch was previously validated end-to-end (full `test` + `test:e2e`) under the same task.

### 2026-05 (pptx-export-charts — Task #311)
- **PPTX download now contains native charts.** The Allocation slide gets a doughnut chart of the group totals (Equity / Real Estate / Bonds / Commodities / Crypto / Cash) alongside the existing per-row table, colored in a navy-tinted palette that matches the rest of the deck. The Monte Carlo slide gets a fan chart of the percentile envelope — a stacked-area band between P10 and P90 (with a tonal break at P50) plus a navy P50 line — alongside the existing percentile / statistics tables.
- **Snapshot wiring.** `buildReportSnapshot` now exposes `monteCarlo.pathSeries: { year, p10, p50, p90 }[]` (start-anchored at year 0, value 100, then one entry per simulated horizon year). Values are rebased to a start-of-100 index, identical to the convention used for `finalP10/P50/P90`. This is additive — existing consumers of the snapshot are unaffected.
- **Export changes.** `exportReportPptx.ts` adds `slide.addChart` calls on the Allocation and Monte Carlo slides. The Allocation table is narrowed to ~7.6" so the doughnut fits on the right half of the slide; the Monte Carlo tables are stacked on the left half so the fan chart takes the right half. Chart colours come from the navy palette (`0F2A4A`, `1F3D5A`, `3A5A80`, `6A89B3`, `A0B8D8`, `C9D6E6`) already used for slide chrome.
- **Fan-chart trick.** PptxGenJS doesn't have a native range-fill chart, so the fan is built with a multi-chart `addChart`: a stacked area chart with three series — `P10` (transparent baseline), `P50-P10` (mid navy), `P90-P50` (light navy) — overlaid with a line chart for the P50 median. The transparent baseline series is hidden via `chartColorsOpacity: 0`, leaving a visible band from P10 to P90 with the median line on top.
- **Tests.** `pnpm run typecheck` and `pnpm --filter @workspace/investment-lab run test:engine` green; no behaviour outside the export path changed.

### 2026-05 (hedge-foreign-bonds-toggle — Task #300)
- **New "Hedge foreign-currency bonds" toggle (default ON).** A dedicated per-portfolio switch separates the FX-hedge decision for fixed income from the legacy all-or-nothing "Currency Hedging" toggle. It lives on Build (under the full-hedge switch), on Explain (next to the existing hedge control), and per slot on Compare. The toggle is hidden for USD-base portfolios (no foreign-currency bond exposure to hedge) and force-on / disabled while full currency hedging is on (full hedge subsumes bond-only).
- **Effective condition.** Bond-only hedging fires iff `hedgeForeignBonds !== false && !includeCurrencyHedging && baseCurrency !== "USD"`. When full hedge is on, the engine treats bonds as hedged anyway, so the toggle's standalone effect collapses into the existing full-hedge behaviour.
- **Engine wiring.**
  - `src/lib/types.ts` — `PortfolioInput.hedgeForeignBonds?: boolean` added. Undefined is treated as ON for backward compatibility with portfolios saved before Task #300.
  - `src/lib/etfs.ts` — the Fixed Income picker routes to the hedged share class (e.g. `FixedIncome-Global-CHF`) when bond-only is effective. Equity and Real Estate stay on the unhedged variant.
  - `src/lib/fees.ts` — `estimateFees` accepts `hedgeForeignBonds` and applies the existing +15 bps hedging surcharge to the FI sleeve only (not Equity / Real Estate). Full-hedge keeps its broader sleeve coverage; turning both flags on does not double-charge.
  - `src/lib/monteCarlo.ts` — `runMonteCarlo` accepts `bondsHedged`, and `muSigmaForKey` cuts the bonds σ by 1pp (with a 1pp floor) when `(hedged || bondsHedged) && baseCurrency !== "USD"`. Equity / commodity / RE σ paths are unchanged.
  - `src/lib/portfolioFile.ts` — `isPortfolioInput` accepts the optional new field so persisted/imported portfolio files survive a round-trip.
  - `src/lib/savedExplainPortfolios.ts` — sanitizer carries `hedgeForeignBonds` through saved Explain workspaces.
  - `src/lib/aiPrompt.ts` — DE+EN prompt builders branch on bond-only vs full hedge so AI follow-up requests state the actual hedge regime.
  - `src/lib/portfolio.ts` — DE+EN learning sentence covers the bond-only branch.
- **UI threading.** Build, Compare (both slots), Explain, PortfolioReport, FeeEstimator and MonteCarloSimulation all read `hedgeForeignBonds` and feed it into fee/MC calls. Compare per-slot threading covers 4 MC sites + 4 FeeEstimator sites + 2 `estimateFees` sites and updates the "Hedging" chip in PortfolioReport.
- **Methodology page** (`src/components/investment/Methodology.tsx`) gains a new sub-block in the hedging section (DE + EN) documenting the bond-only branch and subsumption rule.
- **i18n.** 6 EN + 6 DE keys added (`build.hedgeForeignBonds.label/desc/subsumed`, `explain.hedgeForeignBonds.label`, `report.feature.hedgeBonds`).
- **Tests.** Added regression tests in `tests/engine.test.ts`: (a) ETF picker — FI routes to hedged share class when bond-only is effective, equity stays unhedged; (b) ETF picker — bond-only is a no-op for USD base; (c) `estimateFees` — +15 bps applies to the FI sleeve only (~4.5 bps on a 30%-FI mix) and full-hedge subsumes; (d) `runMonteCarlo` — σ cut on FI only, equity σ untouched; (e) `runMonteCarlo` — bond-only σ cut is a no-op for USD base. `tests/compareMcMetricsAgreement.test.tsx`'s `simplifySlotB` helper now also turns off Slot B's full hedge (and consequently the new bond-hedge subsumption) so the MC ↔ Metrics wiring agreement check stays within its 0.5pp tolerance. `pnpm run typecheck`, `pnpm --filter @workspace/investment-lab run test:engine` and `test:components` green.
...
- **Look-Through geo map aligned with engine country routing (Task #298, 2026-05) — `geomap-align-with-metrics-294`.** `src/lib/geomap.ts` now mirrors the engine's `COUNTRY_TO_EQUITY_KEY` table (`metrics.ts:574-652`) 1:1 — every country the CMA engine routes to an `equity_*` bucket also has a region assignment on the map. Concrete moves: (a) **Ireland** added to `EUROPE_COUNTRIES` (was falling to "Other / Residual" before, e.g. ~1% on a 100% IE00B3YLTY66 portfolio); (b) **Australia, Hong Kong, Singapore, New Zealand** added as a separate `APAC_COUNTRIES` set that classifies as the existing `"Japan"` `RegionKey` (no new RegionKey introduced — mirrors the engine's "no equity_apxj bucket; route to equity_jp" decision from Task #294); (c) **Poland, Greece, Hungary, Czechia, Czech Republic** moved out of `EUROPE_COUNTRIES` into `EM_COUNTRIES` (the engine has been treating them as `equity_em` since Task #294 — the geomap was the last piece holding the old MSCI Europe classification); (d) `EM_COUNTRIES` extended to the full MSCI EM constituent list (Brazil, Mexico, Saudi Arabia, South Africa, Indonesia, Thailand, Malaysia, UAE/United Arab Emirates, Qatar, Kuwait, Egypt, Turkey, Chile, Colombia, Peru, Philippines, Vietnam — all of these previously fell to "Other"); (e) `regionLabel("Japan", …)` widens to **"Japan + Asia-Pacific"** / **"Japan + Asien-Pazifik"** **only when an APAC-developed country (AU/HK/SG/NZ) is actually present in the look-through** — pure Japan-only ETFs/portfolios keep the plain **"Japan"** label so the legend never implies coverage that isn't there. (f) Each region tile now also exposes a `title=` tooltip listing the **actual** countries that contributed to that region (sorted by pct desc), plus any aggregate "Europe"/"EM" roll-up the upstream profile lumped into a single row — so the legend reflects reality, not the map's classification possibilities. (g) The **world choropleth itself** now colours **only** countries that carry a non-zero pct in the look-through (and scales each country's fill opacity by its **own** pct, not its region's pct). A Japan-only ETF lights up Japan and nothing else; Australia / Hong Kong / Singapore / New Zealand stay grey unless they're really in the look-through. Aggregate "Europe"/"EM" rows still surface their weight via the legend tile but no longer paint every classified country in that region. Implementation: `buildRegionWeights` now also returns `regionCountries: Record<RegionKey, Array<{country, pct}>>` and `regionAggregates: Record<RegionKey, number>`; `regionLabel` takes an optional `presentCountries` arg (back-compat default keeps the wider Japan label). The "Other / Residual" tooltip and i18n footnote (`build.geomap.other`, DE+EN) were rewritten to describe the residual as "justETF’s catch-all `Other` row plus any country that neither this map nor the engine's CMA bucket table classifies" — Ireland and Canada carve-out language dropped. User-visible result for a 100% IE00B3YLTY66 portfolio: Other/Residual shrinks from ~14.4% to ~11.7% (the justETF "Other" row only); the Japan tile now reports ~7.3% (Japan 5.7% + Australia 1.7%); Ireland is coloured Europe on the choropleth. New unit coverage in `tests/engine.test.ts` ("Task #298 — geomap aligned with engine country routing") pins all four moves plus a drift-guard that asserts every key of `COUNTRY_TO_EQUITY_KEY` resolves to a non-`"Other"` region — so future engine-table additions cannot silently desync the map again. Methodology page updated (DE+EN) per the Methodology-sync rule.

- **Build → direct PPTX download (Task #310, 2026-05) — `download-pptx`.** The Build tab's **Open as slides / Als Foliendeck öffnen** button is renamed to **Download PPTX / PPTX herunterladen** and now triggers a direct `.pptx` file download (filename `Investment-Report-YYYY-MM-DD.pptx`) instead of opening the Portfolio Report Deck in a new tab. The snapshot still flows through `buildReportSnapshot` (unchanged), but the `localStorage` + `window.open` hand-off is replaced by a new client-side generator at `src/lib/exportReportPptx.ts` built on `pptxgenjs` (added as a devDependency to `@workspace/investment-lab`). The generator produces 10 slides — Cover, Table of Contents, Profile Summary, Key Metrics, Allocation, ETF Implementation, Top 10 Holdings, Monte Carlo, Fees, Methodology/Disclaimer — using a consistent navy/white table theme. Tables that may overflow (ETF list, fees breakdown) use `autoPage: true` with header-repeat so multi-page decks stay readable. A loading toast appears while generation is in progress; success/error toasts confirm the outcome. The legacy `openReportDeck` export was removed from `src/lib/reportSnapshot.ts` (no remaining callers in the lab); the Portfolio Report Deck artifact itself is untouched and still reads `SNAPSHOT_STORAGE_KEY` if any other entry point writes it. i18n keys `build.btn.openSlides` / `build.btn.openSlidesError` are replaced by `build.btn.downloadPptx`, `build.btn.downloadPptxLoading`, `build.btn.downloadPptxSuccess`, `build.btn.downloadPptxError` in both DE and EN. The button's `data-testid` changes from `button-open-report-deck` to `button-download-pptx`. Engine, MC, fees and look-through behaviour are **unchanged** — Methodology sync rule N/A.

- **Build → Portfolio Report Deck handover (Task #306, 2026-05) — `open-in-slides`.** The Build tab gains an **Open as slides / Als Foliendeck öffnen** button next to the two PDF buttons (gated by `output && validation.isValid`, disabled while either PDF export is running). Clicking it serialises a typed snapshot of the current portfolio + Monte-Carlo + fees + lookthrough top-holdings into `localStorage` under `investment-lab.report-snapshot.v1` and opens the deck artifact at `/portfolio-report-deck/` in a new tab. Snapshot is built by `src/lib/reportSnapshot.ts:buildReportSnapshot` and mirrors `PortfolioReport.tsx`'s DetailedSections wiring (same `ILLUSTRATIVE_AMOUNT = 100_000`, same `computeMetrics` / `runMonteCarlo` / `estimateFees` arguments incl. `syntheticUsEffective`, `riskRegime`, `etfImplementation` when look-through is on, `hedged && baseCurrency !== "USD"` for fees). Deck side: new Zod-validated schema `artifacts/portfolio-report-deck/src/data/snapshotSchema.ts` (`schemaVersion: 1`); `reportData.ts` rewritten as a runtime resolver that reads localStorage, validates with Zod, drops invalid payloads silently, and falls back to a curated CHF/Moderate default (`defaultReportData.ts`) so the deck remains demoable standalone and the visual editor / PPTX export keep working. The five previously hard-coded slides (`Allocation`, `Implementation`, `Fees`, `TopHoldings`, `Contents`) are now `.map`-driven; `Allocation` groups rows by `group` field via a fixed `GROUP_ORDER` (equity → realestate → bonds → commodities → crypto → cash) and hides empty groups. New i18n keys `build.btn.openSlides` / `build.btn.openSlidesError` in both languages. Engine, MC, fees and lookthrough behaviour are **unchanged** — Methodology sync rule N/A for this change.
