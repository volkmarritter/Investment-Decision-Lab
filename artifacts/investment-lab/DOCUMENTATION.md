...
- **Storage.** Pinned weights live in `localStorage` under the key `investment-lab.manualWeights.v1` as a flat `Record<string, number>` keyed by `bucket = "${assetClass} - ${region}"` (the same string the engine already produces). They survive reloads, language switches and any change of the form inputs. They are intentionally **not** part of `SavedScenario` — they are a global "how I prefer to see my live portfolio" layer, not a scenario property. The Compare tab does **not** apply the overrides (its `buildPortfolio` calls do not pass them), so A vs B comparisons remain on the natural allocation.
...
- **Audit (kept as `<input type="number">` on purpose)** — documented in the parser's header comment in `src/lib/manualWeights.ts`:
  ...
  - Methodology editors (CMA μ/σ, home-bias, risk-free rate) — admin-only, outside the build/explain hot path; tracked as a separate sweep.
