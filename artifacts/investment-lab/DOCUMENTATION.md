...
| `preferredExchange` | `None \| LSE \| XETRA \| SIX` | ETF listing preference (`None` = engine picks the most liquid European listing per ETF). |
...
### 2026-04-24 (night)
- **Removed "Euronext (Amsterdam)" from the Preferred Exchange dropdown.** The Build tab Select now offers only `None (European listings)`, `LSE`, `XETRA`, `SIX`. Surface area cleaned up consistently: `PreferredExchange` union in `types.ts` shrunk back to `"None" | "LSE" | "XETRA" | "SIX"`; `ExchangeCode` in `etfs.ts` mirrored; all 16 `Euronext: { ticker: ... }` listing entries stripped from `CATALOG` (no ETF used Euronext as `defaultExchange`, so removal is non-destructive — the engine still picks each ETF's default European listing when `preferredExchange === "None"`); `aiPrompt.ts` `EXCHANGE_LINE` no longer carries Euronext lines (EN/DE); `i18n.tsx` lost the `build.preferredExchange.option.euronext` key (EN/DE) and the tooltip wording reverts to "LSE, XETRA or SIX" / "LSE, XETRA oder SIX"; `BuildPortfolio.tsx` Select no longer renders the Euronext `SelectItem`. The Euronext-specific engine test was removed (suite back to 97 passing tests). Typecheck clean. §3 input-table and §5 documentation entries reverted to the LSE / XETRA / SIX list.
